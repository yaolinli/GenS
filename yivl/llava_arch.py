...
from gauge.models.arch.yivl.clip_encoder.builder import build_vision_tower
from gauge.models.arch.yivl.constants import (
...
from gauge.models.arch.yivl.multimodal_projector.builder import build_vision_projector
...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(
        self, vit_path, mlp_path, projector_type="mlp2x_gelu_Norm"
    ):
        self.config.mm_vision_tower = vit_path
        ...
        def get_attn_w(weights, keyword):
            return {
                "PerceiverResample." + k.split(keyword + ".")[1]: v
        ...
        if projector_type == "attention":
            fixed_state_dict = {
                key.replace("mm_projector.", ""): value
                for key, value in mm_projector_weights.items()
            }
            self.mm_projector.load_state_dict(fixed_state_dict)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            N, patch_num, C, H, W = images.shape
            concat_images = images.view(-1, C, H, W).contiguous()
            image_features = self.encode_images(concat_images)
            image_features = image_features.view(
                -1, patch_num, image_features.shape[-2], image_features.shape[-1]
            ).contiguous()  # N, patch_num, image_token_len, dim
            image_features = image_features.reshape(
                image_features.shape[0],
                image_features.shape[1] * image_features.shape[2],
                image_features.shape[3],
            )  # N, patch_num * image_token_len, dim
        else:
