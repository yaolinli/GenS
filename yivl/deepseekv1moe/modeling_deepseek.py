...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    warnings.warn(
        "Calling `transformers.models.Deepseek.modeling_Deepseek._prepare_4d_attention_mask` "
...
def _make_causal_mask(
    ...
    warnings.warn(
        "Calling `transformers.models.Deepseek.modeling_Deepseek._make_causal_mask` "
        "is deprecated and will be removed in v4.37. Use "
        "`transformers.models.Deepseek.modeling_Deepseek.AttentionMaskConverter._make_causal_mask"
    )
...
class DeepseekDynamicNTKScalingRotaryEmbedding(DeepseekRotaryEmbedding):
    """
    DeepseekRotaryEmbedding extended with Dynamic NTK scaling.
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            y = (y.view(*topk_weight.shape, -1) * topk_weight.unsqueeze(-1)).sum(
                dim=1, dtype=hidden_states.dtype
            )
            y = y.view(*orig_shape)
...
class DeepseekFlashAttention2(DeepseekAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # while what is needed here is bottom-right alignement,
        # that was made default for flash_attn>=2.1.
        # This attribute is used to handle this difference.
    ...
    def _flash_attention_forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor`):
                ...
                `(batch_size, seq_len)` where 0 stands for the
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # For details, please see the comment in DeepseekFlashAttention2 __init__.
...
class DeepseekSdpaAttention(DeepseekAttention):
    """
    Deepseek attention module using torch.nn.functional.scaled_dot_product_attention.
    This module inherits from
    `DeepseekAttention` as the weights of the module stays untouched.
    The only changes are on the forward pass to adapt to
    SDPA API.
    """
    ...
    def forward(
        ...
        if output_attentions:
            ...
            # `model.config.attn_implementation = "manual"` once this is implemented.
            logger.warning_once(
                "DeepseekModel is using DeepseekSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention`"
        ...
        **kwargs,
        ...
        Args:
            ...
            attention_mask (`torch.FloatTensor`, *optional*):
                ...
                if flash attention is used or `(batch_size, 1,
            ...
            use_cache (`bool`, *optional*):
                ...
                are returned and can be used to speed up decoding (see `past_key_values`).
    ...
    def forward(
        ...
        Args:
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                Indices should either be in `[0, transformers.,
